...
var getProcessed = function (processing, value) {
...
Plugin.prototype.resolve = function (parentDeps, callback) {
	...
	if (this.dependencies.length === 0) {
		this.res = getProcessed( this.processing, this.raw.wiretree( ));
		this.resolved = true;
		return callback();
	}
	// async plugin declaration
	var wtDone = function (mod) {
		self.res = getProcessed( self.processing, mod );
		self.resolved = true;
		callback();
	};
	...
	var count = counter( self.dependencies.length, function () {
		...
		if (!self.isAsync) {
			self.res = getProcessed( self.processing, self.raw.wiretree.apply( {}, deps ));
			self.resolved = true;
			return callback();
		}

...
var resolver = function (plugin, sliceList) {
	var deps = [],
		data, self = this;
...
_get = function (key, listKeys, noMain) {
	...
	if (typeof plugin.res === 'undefined') {
		if (plugin.async === true) {
			throw new Error( 'Error getting: ' + plugin.key + '. Can\'t get async plugins before resolve');
		}
		// if not served or not ready to serve
...
Wiretree.prototype.load = function (route, key, options) {
	...
	route = path.resolve( this.rootPath, route );
